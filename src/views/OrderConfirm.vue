      ...
      <div class="checkout-order">
        ...
        <div class="order-foot-wrap">
          <div class="prev-btn-wrap">
            <button class="btn btn--m" @click="previousPage">Previous</button>
          </div>
...
<script>
  ...
  import axios from 'axios'
    export default {
        ...
        methods: {
          getCheckedProducts() {
            axios.get('/users/cart/checked').then((response) => {
              let res = response.data;
              if(res.code == 0){
                this.list = res.result;
                this.itemTotal = 0;
                for(let i=0; i<this.list.length; i++){
                  this.itemTotal += this.list[i].salePrice * this.list[i].productNum;
                }
                this.orderTotal = this.itemTotal + this.shipping - this.discount + this.tax;
              }
            });
          },
          previousPage() {
          ...
          payMent() {
            axios.post('/users/payMent',{orderTotal: this.orderTotal, addressId: this.$route.query.addressId}).then((response) => {
              let res = response.data;
              if(res.code == 0){
                this.$router.push({

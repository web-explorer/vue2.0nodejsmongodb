        ...
        <div class="addr-list-wrap">
          <div class="addr-list">
            <ul>
              ...
              <li v-for="(item, index) in addressListFilter" v-bind:class="{check: item.addressId == selectedAddressId}" @click="chooseAddress(item.addressId)">
                <dl>
                  <dt>{{item.userName}}</dt>
                ...
                <div class="addr-opration addr-del">
                  <a href="javascript:;" class="addr-del-btn" @click="startDelete(index)">
                    <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                  </a>
                </div>
                <div class="addr-opration addr-default" v-if="!item.isDefault">
                  <a href="javascript:;" class="addr-set-default-btn" @click="setDefault(index)"><i>Set default</i></a>
                </div>
...
<!--          <a class="btn btn&#45;&#45;m btn&#45;&#45;red">Next</a>-->
          <router-link class="btn btn--m btn--red" v-bind:to="'/orderConfirm?addressId=' + selectedAddressId">Next</router-link>
        </div>
      </div>
    </div>
    <modal :md-show="delComfirm" v-bind:mark="delComfirm" @close="close">
      <p slot="message">确定删除这个地址吗？</p>
      <div slot="btnGroup">
  ...
  import axios from 'axios'
    export default {
      ...
      methods: {
          ...
          setDefault(index) {
            axios.post('/users/setDefaultAddress', {index: index}).then((response) => {
              let res = response.data;
              if(res.code == 0){
                this.getAddressList();
              }else{
          ...
          del() {
            axios.post('/users/deleteAddress', {index: this.deleteFlag}).then((response) => {
              let res = response.data;
              if(res.code == 0){
